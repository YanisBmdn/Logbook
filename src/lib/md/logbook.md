...
Instead of reporting daily, I'll try reporting days with major advancements / findings. This will make this logbook less of a "choir".

Came up with a pretty convincing MVP (the pipeline was only made using a notebook). I was genuinely immensely impressed by the quality of the graph generated by Claude AI.
Here's the graph that Claude created for the following query : 
![Temperature evolution graph made by Claude](/images/1115-graph.webp)

Here's his analysis :
```
Looking at this temperature analysis for Nagoya (2010-Present), there are several key observations:
Long-term Warming Trend:
The red line (yearly moving average) shows a gradual upward trend from around 15°C in 2010 to about 16°C in 2024
This represents roughly 1°C of warming over 14 years, which is a significant increase
Seasonal Patterns:
The blue line shows consistent seasonal variations with:
Summer peaks around 25-30°C
Winter lows around 0-5°C
These patterns remain regular throughout the period
Temperature Anomalies (bottom heatmap):
More red colors (warmer than baseline) appear in recent years (2020 onwards)
The intensity of warm anomalies (darker red) seems to be increasing in the later years
Earlier years (2010-2015) show more blue colors, indicating cooler temperatures relative to the baseline
```

I was congratulated for my work, which makes me gain confidence for pursuing this theme and subject ! I hope I'll be able to bring some significant
system to the world.

## Fri. 11-08-2024

The goal of the research will be to provide a tool that can automatically retrieve automatically retrieve data relevant to a user's request.

For now, I have thought about delegating these things to an LLM.

- Which data source to fetch (via open, public APIs)
- Which parameters to use (localisation, date/period, etc.)

One of the feature that could be cool is the comparison of the current studied area with other close / related / similar areas.

Also found out about [Breezometer](https://docs.breezometer.com/api-documentation/introduction/#schema) It's the service used for instance by Apple's weather app to display air quality metrics. However, I might go with [Openaq](https://docs.openaq.org/about/about) as it appears to be a 100% open data solution.

My sensei also told me about two forum tools I could use **Decidim** and **Discourse**. I'll have to learn more about them and how I could use them.
